...
func seedChains() {
	defaults := []mdb.Chain{
		...
		{Network: mdb.NetworkXrpl, DisplayName: "XRP Ledger", Enabled: true, MinConfirmations: 1, ScanIntervalSec: 5},
	}
...
func seedChainTokens() {
	...
	defaults := []mdb.ChainToken{
		...
		// Ethereum — RLUSD (ERC-20, issued by Ripple). Verify contract address & decimals on-chain before use.
		{Network: mdb.NetworkEthereum, Symbol: "RLUSD", ContractAddress: "0x8292bB45bf1Ee4d140127049BEEa21e3b4D47Eea", Decimals: 6, Enabled: true},
		// XRP Ledger — RLUSD issued currency. ContractAddress holds the issuer account.
		// Verify issuer address on https://xrpscan.com before use.
		{Network: mdb.NetworkXrpl, Symbol: "RLUSD", ContractAddress: "rMxCKbEDwqr76QuheSUMdEGf4B9xJ8m5De", Decimals: 6, Enabled: true},
	}
	if err := Mdb.Clauses(clause.OnConflict{DoNothing: true}).Create(&defaults).Error; err != nil {
...
func defaultRpcNodes() []mdb.RpcNode {
	defaults := []mdb.RpcNode{
		...
		{Network: mdb.NetworkXrpl, Url: "wss://xrplcluster.com", Type: mdb.RpcNodeTypeWs, Weight: 1, Enabled: true, Purpose: mdb.RpcNodePurposeGeneral, Status: mdb.RpcNodeStatusUnknown},
		{Network: mdb.NetworkXrpl, Url: "wss://s1.ripple.com", Type: mdb.RpcNodeTypeWs, Weight: 1, Enabled: true, Purpose: mdb.RpcNodePurposeGeneral, Status: mdb.RpcNodeStatusUnknown},
